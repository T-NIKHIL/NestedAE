...
def get_module_input_dim(connect_to, nn_params_dict, variable_shapes):
    ...
    for inp in connect_to:
        ...
        # Case where input to layer is ouput from last layer of connected submodule
            ...
            input_dim = submodule_dicts[inp]['output_dim']

        tot_input_dim += input_dim

    return tot_input_dim 

def set_layer_init(layer_list, submodule_dict):
    ...
    if submodule_dict['hidden_activation'] == 'relu':
        hidden_gain = math.sqrt(2)
    elif submodule_dict['hidden_activation'] == 'tanh':
        hidden_gain = 5.0 / 3
    else:
        hidden_gain = 1
    ...
    if 'output_activation' in list(submodule_dict.keys()):
        if submodule_dict['output_activation'] == 'relu':
            out_gain = math.sqrt(2)
        if submodule_dict['output_activation'] == 'tanh':
            out_gain = 5.0 / 3
    else:
        out_gain = 1

    kernel_init_type = submodule_dict['layer_kernel_init']
    bias_init_type = submodule_dict['layer_bias_init']
    ...
    for i, layer in enumerate(layers):
        if '_' in kernel_init_type:
            ...
            if scheme == 'kaiming' and distribution == 'uniform':
                ...
                if i == submodule_dict['hidden_layers']:
                    torch.nn.init.xavier_uniform_(layer.weight, gain=out_gain)
                else:
                    torch.nn.init.xavier_uniform_(layer.weight, gain=hidden_gain)
            else:
                if i == submodule_dict['hidden_layers']:
                    torch.nn.init.xavier_normal_(layer.weight, gain=out_gain)
                else:
                    torch.nn.init.xavier_normal_(layer.weight, gain=hidden_gain)
    ...
    for i, layer in enumerate(layers):
        if '_' in bias_init_type:
            ...
            if scheme == 'xavier' and distribution == 'uniform':
                if i == submodule_dict['hidden_layers']:
                    torch.nn.init.xavier_uniform_(layer.bias, gain=out_gain)
                else:
                    torch.nn.init.xavier_uniform_(layer.bias, gain=hidden_gain)
            else:
                if i == submodule_dict['hidden_layers']:
                    torch.nn.init.xavier_normal_(layer.bias, gain=out_gain)
                else:
                    torch.nn.init.xavier_normal_(layer.bias, gain=hidden_gain)
        elif bias_init_type == 'zeros':
            torch.nn.init.zeros_(layer.bias)
        elif bias_init_type == 'uniform':
            in_dim = layer.weight.size(1)
            torch.nn.init.uniform_(layer.bias, a=-math.sqrt(1/in_dim), b=math.sqrt(1/in_dim))
        else:
...
def create_optimizer_object(submodules, nn_train_params_dict):
    ...
    if optimizer_type == 'adam':
        ...
        module_parameters = submodules.parameters()
        sgd_optimizer = torch.optim.SGD(module_parameters,
                                        lr=nn_train_params_dict['optimizer']['lr'],
