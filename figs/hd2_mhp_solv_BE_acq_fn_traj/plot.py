...
which_trial = 2
random_state = 42
which_latent_hist_file = 'selected_latent_hist_nestedhd_min.npy'
save_video_filename = f'mhp_solv_BE_acq_fn_traj_trial_{which_trial + 1}_nestedhd_min.mp4'
maximize = False
...
latents_all = from_numpy(np.load('latents_stand.npy'))
selected_latent_hist = np.load(which_latent_hist_file, allow_pickle=True)

sel_data_umap_list = []
for selected_latent in selected_latent_hist[which_trial, :]:
    match_idx = (latents_all == from_numpy(selected_latent)).all(dim=1).nonzero(as_tuple=False).item()
...
for i in range(len(sel_data_umap)):
    ...
    if maximize:
        ax.scatter(umap_dim1_opt_be, umap_dim2_opt_be, color='black', marker='*', s=10, label='Candidate with max. BE')
    else:   
