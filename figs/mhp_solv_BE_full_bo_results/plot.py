...
if __name__ == "__main__":
    ...
    model_dir_nestedae = '../../runs/ae2_bandgaps_THEN_perov_solv_BE/fold3_soft_constraints_7D'
    latent_dim_nestedae = 7
    ...
    #################################################################
    # Hetero decoder 2 Parameters
    #################################################################

    model_dir_nestedhd = '../../runs/hd2_bandgaps_THEN_perov_solv_BE/fold7_soft_constraints_6D'
    latent_dim_nestedhd = 6
    module_params_nestedhd = {'name':'AE1',
                            'modules':{
                                
                                'encoder':{
                                    'input_dim':16,
                                    'output_dim':latent_dim_nestedhd, 
                                    'hidden_dim':25,
                                    'hidden_layers':1, 
                                    'hidden_activation':None, 
                                    'output_activation':torch.nn.Tanh(), 
    ...
    latent_col_names = ['l0', 'l1', 'l2', 'l3', 'l4', 'l5', 'l6', 'l7', 'l8', 'l9', 'l10'] # For NestedAE
    # latent_col_names = ['l0', 'l1', 'l2', 'l3', 'l4', 'l5', 'l6', 'l7', 'l8', 'l9'] # For NestedHE
    target = ['Target']
    ...
    nestedae_or_nestedhd_feat_sel = 'nestedae' # None or 'nestedae' or 'nestedhe'
    test_size = 0.96
    bo_n_trials = 200
    bo_n_updates = 10
    bo_n_batches = 100
    ...
    verbose = True
    # Plot parameters
    y_axis_tick_range = np.arange(0, 34, 2)
    show_mean = False
    show_median = False
    ...
    which_files_to_load = ['best_obs_val_all_trials_tensor_SimpleBO.pt',
                           ...
                           'best_obs_val_all_trials_tensor_NestedHE.pt']
    ...
    if not load_data:
        ...
        if nestedae_or_nestedhd_feat_sel == 'nestedae':
        ...
        elif nestedae_or_nestedhd_feat_sel == 'nestedhe':
            loaded_model = AE(module_params_nestedhd)
            loaded_model.load_state_dict(torch.load(model_dir_nestedhd))
            loaded_model.eval()
            with torch.no_grad():
                for i, layer in enumerate(loaded_model.ae_modules['encoder']):
                    if i == 0:
            ...
            latents_std_dev_dict = {}
            desc_stand = copy.deepcopy(latents)
            for dim in range(latent_dim_nestedhd):
                mean = latents[:, dim].mean()
                latents_mean_dict[dim] = mean
                std_dev = latents[:, dim].std()
                latents_std_dev_dict[dim] = std_dev
    ...
    for method, best_obs_all_trials in zip(list_of_methods, list_of_best_obs_all_trials_tensor):
        ...
        ax.text(text_pos[k]-0.38, median_val-0.5, f"{median_val:.1f}", ha='center', fontsize=8, color='black', fontweight='bold')
        if show_mean:
            # Show mean as a horizontal dotted red line
            ax.axhline(mean_val, color='red', linestyle='--', linewidth=2, label='Mean')
            ax.text(-0.38, mean_val + 0.6, f"Mean: {mean_val:.1f}", ha='center', fontsize=8, color='black', fontweight='bold')
        if show_median:
