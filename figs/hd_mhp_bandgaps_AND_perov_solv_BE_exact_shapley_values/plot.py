...
data = pd.read_csv('data_for_BE.csv')
...
#                   r'$\textrm{solv}_{\textrm{density}}$', r'$\textrm{solv}_{\textrm{dielec.}}$', r'$\textrm{solv}_{\textrm{GDN}}$', r'$\textrm{solv}_{\textrm{dipole}}$', r'$\textrm{solv}_{\textrm{MV}}$', r'$\textrm{solv}_{\textrm{UMBO}}$']
mean_abs_shapley_values = []

for feat in feats:
    mean_abs_shapley_values.append(np.mean(np.abs(data[feat])))

mean_abs_shapley_values, feats_for_plot = zip(*sorted(zip(mean_abs_shapley_values, feats), reverse=False))
...
plt.tight_layout()
plt.savefig('hd_mhp_binding_energy_exact_shapley_values.pdf', bbox_inches='tight', dpi=300)
