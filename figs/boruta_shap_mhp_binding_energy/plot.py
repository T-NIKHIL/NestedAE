...
if __name__ == "__main__":
    ...
    rename_columns = ['A_IONRAD', 'A_MASS', 'A_DPM',
                      'X_IONRAD', 'X_MASS', 'X_EA', 'X_IE', 'X_En', 'X_AN',
                      'SOLV_DENS', 'SOLV_DIEL', 'SOLV_GDN', 'SOLV_DPM', 'SOLV_MV', 'SOLV_UMBO']
    ...
    #     desc_values = data[data['Features'] == desc]
    #     # Replace any NAN values with 0.0
    #     desc_values['value'] = desc_values['value'].fillna(0.0)
    #     mean_abs_z_scores[rename_col] = np.mean(np.abs(desc_values['value'].to_numpy()))
    ...
    # Sort the keys in mean_abs_shap_values in ascending order of their values
    # sorted_items = sorted(mean_abs_shap_values.items(), key=lambda x: x[1])
    # rename_columns, mean_abs_shap_values = zip(*sorted_items)
    # mean_abs_shap_values = list(mean_abs_shap_values)
    ...
    # ax.barh(rename_columns, mean_abs_shap_values, color='skyblue')
